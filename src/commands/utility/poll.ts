...
import {checkPermissions, resolveDate} from "../../functions.js";
...
export const command: Command = {
	data: new SlashCommandBuilder()
		...
		.addSubcommand((subcommand) =>
			subcommand
				...
				.addStringOption((option) =>
					option.setName("choice-1").setDescription("The first option."),
				)
				.addStringOption((option) =>
					option.setName("choice-2").setDescription("The second option."),
				)

				.addStringOption((option) =>
					option.setName("choice-3").setDescription("The third option."),
				)
				.addStringOption((option) =>
					option.setName("choice-4").setDescription("The fourth option."),
				)
				.addStringOption((option) =>
					option.setName("choice-5").setDescription("The fifth option."),
				)
				.addStringOption((option) =>
					option.setName("choice-6").setDescription("The sixth option."),
				)
				.addStringOption((option) =>
					option.setName("choice-7").setDescription("The seventh option."),
				)
				.addStringOption((option) =>
					option.setName("choice-8").setDescription("The eighth option."),
				)
				.addStringOption((option) =>
					option.setName("choice-9").setDescription("The ninth option."),
				)
				.addStringOption((option) =>
	...
	/* .addSubcommand((subcommand) =>
		...
		if (interaction.options.getSubcommand() === "create") {
			let duration: string | null | number =
				interaction.options.getString("duration");

			if (duration) {
				try {
					duration = new Parser().evaluate(
						`${resolveDate(
							/^in.+/.test(duration.trim())
								? duration.match(/(?<=^in).+/)[0]
								: duration,
						)}`,
					);
			...
			if (!permissions.value) {
				return await interaction.reply(
					new ErrorMessageBuilder(permissions.message, true),
				);
			}

			await interaction.deferReply({
				embeds: [new EmbedBuilder().setTitle("📩 Sending...").setColor("Blue")],
				ephemeral: true,
			});
			...
			await interaction.editReply(
				new SuccessMessageBuilder(
					`Successfully created and sent the poll in ${channel}.`,
					true,
				),
			);
		}
